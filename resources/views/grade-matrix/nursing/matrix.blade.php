    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                <a href="{{ route('nursing.recalculate-grades', $subject) }}" 
                   ...
                   onclick="return confirm('This will recalculate all term grades for this subject using the nursing formula. Continue?');">
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6" x-data="{
    ...
    get isLecture() {
        return this.subjectType === 'lecture';
    },
    get isLab() {
    ...
    saveFinalRatingWeights() {
        ...
        .then(data => {
            if (data.success) {
                alert('Final rating configuration saved successfully! Page will reload.');
            ...
            <div class="text-left">
                <div class="text-sm text-gray-600 font-medium flex items-center gap-2">
                    ...
                    <span class="px-2 py-0.5 text-xs font-semibold rounded-full" 
                          :class="isLecture ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'"
                          x-text="isLecture ? 'LECTURE' : 'LAB'"></span>
                </div>
                <div class="text-lg font-semibold text-gray-900">
                    <span x-show="isLecture">
                        ...
                        <span class="text-orange-600"> + Comprehensive Exam</span>
                    </span>
                    <span x-show="isLab">
                        <span class="text-blue-600" x-text="prelimWeight + '%'"></span> Prelims + 
                        <span class="text-green-600" x-text="midtermWeight + '%'"></span> Midterm + 
    ...
    <div x-show="expanded" x-cloak x-transition class="px-6 pb-6 border-t border-gray-200">
        <div class="mt-4 p-4 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg border-2 border-orange-300">
            ...
            <form @submit.prevent="saveFinalRatingWeights()" class="space-y-4">
                ...
                <div class="p-4 bg-white rounded-lg border-2 border-orange-200">
                    <label class="block text-sm font-medium text-gray-700 mb-3">
                        <i class="fas fa-flask mr-1 text-orange-600"></i>
                        Subject Type
                    </label>
                    <div class="flex gap-4">
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" x-model="subjectType" value="lecture" class="mr-2 text-blue-600 focus:ring-blue-500">
                            <span class="flex items-center gap-2 px-4 py-2 rounded-lg border-2 transition-all"
                                  :class="isLecture ? 'border-blue-500 bg-blue-50 text-blue-900 font-semibold' : 'border-gray-300 bg-white text-gray-700'">
                                <i class="fas fa-chalkboard-teacher"></i>
                                Lecture (with Comprehensive Exam)
                            </span>
                        </label>
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" x-model="subjectType" value="lab" class="mr-2 text-green-600 focus:ring-green-500">
                            <span class="flex items-center gap-2 px-4 py-2 rounded-lg border-2 transition-all"
                                  :class="isLab ? 'border-green-500 bg-green-50 text-green-900 font-semibold' : 'border-gray-300 bg-white text-gray-700'">
                                <i class="fas fa-flask"></i>
                                Laboratory (no Comprehensive Exam)
                            </span>
                        </label>
                    </div>
                </div>
...
@else
    @php
        ...
        // Check if subject is lecture or lab
        $isLecture = ($finalRatingConfig['subject_type'] ?? 'lecture') === 'lecture';
        $isLab = !$isLecture;
    ...
    <!-- Grade Matrix Table - NURSING SPECIFIC (Simplified) -->
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-medium uppercase tracking-wider bg-purple-50 text-purple-900 border-l border-gray-300">
                            <div class="flex flex-col items-center">
                                ...
                                <span>Finals Grade</span>
                            </div>
                        </th>
                        @if($isLecture)
                            <!-- Final Grade (80%) - Lecture Only -->
                            <th class="px-6 py-4 text-center text-xs font-medium uppercase tracking-wider bg-indigo-50 text-indigo-900 border-l border-gray-300">
                                <div class="flex flex-col items-center">
                                    <i class="fas fa-chart-line mb-1"></i>
                                    <span>Final Grade<br>(80%)</span>
                                </div>
                            </th>
                            <!-- Comprehensive Exam (20%) - Lecture Only -->
                            <th class="px-6 py-4 text-center text-xs font-medium uppercase tracking-wider bg-orange-50 text-orange-900 border-l border-gray-300">
                                <div class="flex flex-col items-center">
                                    <i class="fas fa-graduation-cap mb-1"></i>
                                    <span>Comprehensive<br>Exam (20%)</span>
                                </div>
                            </th>
                        @endif
                        <!-- Final Rating -->
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($subject->studentMappings as $index => $studentMapping)
                        ...
                        <tr class="hover:bg-gray-50 {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-25' }}">
                            ...
                            <!-- Prelim Grade -->
                            <td class="px-6 py-4 text-center border-l border-blue-300 bg-blue-50">
                                @if($prelimGrade && $prelimGrade->term_grade !== null)
                                    <div class="text-2xl font-bold text-blue-900">
                                        {{ number_format((float)$prelimGrade->term_grade, 2) }}
                                    </div>
                                    <div class="text-xs text-gray-600 mt-1">
                                        @if($prelimGrade->term_grade >= 75)
                            ...
                            <!-- Midterm Grade -->
                            <td class="px-6 py-4 text-center border-l border-green-300 bg-green-50">
                                @if($midtermGrade && $midtermGrade->term_grade !== null)
                                    <div class="text-2xl font-bold text-green-900">
                                        {{ number_format((float)$midtermGrade->term_grade, 2) }}
                                    </div>
                                    <div class="text-xs text-gray-600 mt-1">
                                        @if($midtermGrade->term_grade >= 75)
                                            <span class="text-green-600 font-semibold">
                                                <i class="fas fa-check-circle mr-1"></i>Passing
                                            </span>
                                        @else
                            ...
                            <!-- Finals Grade -->
                            <td class="px-6 py-4 text-center border-l border-purple-300 bg-purple-50">
                                @if($finalsGrade && $finalsGrade->term_grade !== null)
                                    <div class="text-2xl font-bold text-purple-900">
                                        {{ number_format((float)$finalsGrade->term_grade, 2) }}
                                    </div>
                                    <div class="text-xs text-gray-600 mt-1">
                                        @if($finalsGrade->term_grade >= 75)
                                            <span class="text-green-600 font-semibold">
                                                <i class="fas fa-check-circle mr-1"></i>Passing
                                            </span>
                                        @else
                            ...
                            @if($isLecture)
                                <!-- Final Grade (80%) - Lecture Only -->
                                <td class="px-6 py-4 text-center border-l border-indigo-300 bg-indigo-50">
                                    @php
                                        // Calculate 80% of the final grade
                                        $finalGrade80 = $finalGrade !== null ? ((float)$finalGrade * 0.80) : null;
                                    @endphp
                                    @if($finalGrade80 !== null)
                                        <div class="space-y-1">
                                            <div class="text-2xl font-bold text-indigo-900">
                                                {{ number_format((float)$finalGrade80, 2) }}
                                            </div>
                                            <div class="text-xs text-gray-600">
                                                100%: {{ number_format((float)$finalGrade, 2) }}
                                            </div>
                                        </div>
                                    @else
                                        <span class="text-gray-400 text-sm">—</span>
                                ...
                                <!-- Comprehensive Exam (20%) - Lecture Only -->
                                <td class="px-4 py-4 text-center border-l border-orange-200 bg-orange-25">
                                    @php
                                        // Get comprehensive exam score from subject's comprehensive_exam_scores (raw score 0-100)
                                        $comprehensiveExamRaw = $subject->comprehensive_exam_scores[$studentMapping->id] ?? null;
                                        // Transmute the comprehensive exam score: (score / 100) * 60 + 40
                                        $comprehensiveExamTransmuted = $comprehensiveExamRaw !== null 
                                            ? (((float)$comprehensiveExamRaw / 100) * 60 + 40) 
                                            : null;
                                        // Calculate 20% of the transmuted comprehensive exam
                                        $comprehensiveExam20 = $comprehensiveExamTransmuted !== null 
                                            ? ((float)$comprehensiveExamTransmuted * 0.20) 
                                            : null;
                                    @endphp
                                    @if($comprehensiveExam20 !== null)
                                        <div class="space-y-1">
                                            <div class="text-2xl font-bold text-orange-800">
                                            {{ number_format((float)$comprehensiveExam20, 2) }}
                                            </div>
                                            <div class="text-xs text-gray-600">
                                                Transmuted: {{ number_format((float)$comprehensiveExamTransmuted, 2) }}
                                            </div>
                                            <div class="text-xs text-gray-500">
                                                Raw: {{ number_format((float)$comprehensiveExamRaw, 0) }}
                                            </div>
                                        </div>
                                    @else
                                        <span class="text-gray-400 text-sm">—</span>
                            ...
                            <!-- Final Rating -->
                            <td class="px-6 py-4 text-center border-l border-gray-300 bg-teal-25">
                                @php
                                    $finalRating = null;
                                    if ($isLecture) {
                                        // Lecture: Final Rating = Final Grade (80%) + Comprehensive Exam (20%)
                                        if (isset($finalGrade80) && $finalGrade80 !== null && isset($comprehensiveExam20) && $comprehensiveExam20 !== null) {
                                            $finalRating = (float)$finalGrade80 + (float)$comprehensiveExam20;
                                        }
                                    } else {
                                ...
                                @if($finalRating !== null)
                                    <div class="space-y-1">
                                        <div class="text-3xl font-bold text-teal-900">
                                            {{ number_format((float)$finalRating, 2) }}
                                        </div>
        ...
        <div class="flex flex-col gap-4">
            <div class="mb-2">
                <h3 class="text-lg font-bold text-gray-900 flex items-center mb-3">
                    ...
                    Nursing Matrix Computation Formulas
                </h3>
            </div>
            
            <!-- Component Formulas -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                <div class="flex flex-col">
                    <span class="text-xs font-semibold text-blue-900 uppercase mb-1">Activities</span>
                    <span class="text-sm text-gray-700 font-mono">(Score / Total × 60 + 40) × 0.15</span>
                </div>
                <div class="flex flex-col">
                    <span class="text-xs font-semibold text-blue-900 uppercase mb-1">Quizzes</span>
                    <span class="text-sm text-gray-700 font-mono">(Score / Total × 60 + 40) × 0.25</span>
                </div>
                <div class="flex flex-col">
                    <span class="text-xs font-semibold text-blue-900 uppercase mb-1">Exam</span>
                    <span class="text-sm text-gray-700 font-mono">(Score / Total × 60 + 40) × 0.60</span>
                </div>
            </div>
            ...
            <div class="p-4 bg-green-50 rounded-lg border border-green-200">
                <div class="flex items-center gap-2 mb-2">
                    <i class="fas fa-equals text-green-600"></i>
                    <span class="text-sm font-semibold text-green-900">Term Grade (Prelim/Midterm/Finals)</span>
                </div>
                <span class="text-sm text-gray-700 font-mono">Term Grade = Activities + Quizzes + Exam</span>
            </div>
            
            <!-- Final Grade Formula (100%) -->
            <div class="p-4 bg-purple-50 rounded-lg border border-purple-200">
                <div class="flex items-center gap-2 mb-2">
                    <i class="fas fa-chart-line text-purple-600"></i>
                    <span class="text-sm font-semibold text-purple-900">Final Grade (100%)</span>
                </div>
            ...
            <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-sm font-semibold text-indigo-900">Final Grade (80% for Final Rating)</span>
                </div>
                <span class="text-sm text-gray-700 font-mono">Final Grade (80%) = Final Grade × 0.80</span>
            </div>
            
            <!-- Comprehensive Exam Formula -->
            <div class="p-4 bg-orange-50 rounded-lg border border-orange-200">
                <div class="flex items-center gap-2 mb-2">
                    <i class="fas fa-graduation-cap text-orange-600"></i>
                    <span class="text-sm font-semibold text-orange-900">Comprehensive Exam (20% for Final Rating)</span>
                </div>
                <div class="space-y-1">
                    <div class="text-sm text-gray-700 font-mono">Step 1: Transmuted Score = (Raw Score / 100 × 60 + 40)</div>
                    <div class="text-sm text-gray-700 font-mono">Step 2: Comprehensive Exam (20%) = Transmuted Score × 0.20</div>
                </div>
            </div>
            ...
            <div class="p-4 bg-teal-50 rounded-lg border-2 border-teal-300">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-sm font-semibold text-teal-900">Final Rating</span>
                </div>
                <span class="text-sm text-gray-700 font-mono font-bold">Final Rating = Final Grade (80%) + Comprehensive Exam (20%)</span>
            </div>
            
            <!-- Passing Criteria -->
            <div class="flex items-center justify-end gap-4 text-sm pt-4 border-t border-gray-200">
                <div class="flex items-center">
...
<div x-data="{ 
    ...
    proceedExport() {
        ...
        if (this.exportType === 'pp') {
            url = '{{ route('grades.export.pp', $subject) }}' + '?matrix_type={{ $matrixType }}';
        ...
        // Trigger download (subject_type will be read from database in controller)
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="border rounded-lg p-4">
                                        ...
                                        <div class="ml-8 space-y-2">
                                            <div @click="promptForDean('pp', '', 'lecture')"
                                            ...
                                            <div @click="promptForDean('pp', '', 'lab')"
                                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded transition-colors cursor-pointer">
