...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('grade-matrix.general-education.matrix', $subject) }}" class="text-white px-4 py-2 rounded-lg font-medium transition-colors"
                ...
                <a href="{{ route('grade-matrix.general-education') }}" class="px-4 py-2 rounded-lg font-medium transition-colors border"
                   ...
                   onmouseout="this.style.backgroundColor='white'; this.style.color='#08695A';">
                    <i class="fas fa-arrow-left mr-2"></i>Back to General Education Matrix
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border mb-6" x-data="{ 
        ...
        <div class="border-b border-gray-200">
            <nav class="flex space-x-8 px-6" aria-label="Tabs">
                @foreach(['prelim', 'midterm', 'finals'] as $termKey)
                    <a href="{{ route('grade-matrix.general-education.term', ['subject' => $subject, 'term' => $termKey]) }}"
        ...
        <div class="p-6 border-b border-gray-200 bg-gray-50 hidden" x-data="{ 
            ...
            termConfig: {
                // General Education Matrix always uses these defaults
            ...
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Default: 66.67% CS + 33.33% Exam (editable)</p>
                </div>
                                ...
                                @foreach($subject->studentMappings as $studentMapping)
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-center bg-teal-25 border-l border-teal-200">
                                            ...
                                            @if($totalPossible > 0)
                                                @php
                                                    // General Education uses transmuted formula: (score/total) × 50 + 50
                                                    $transmutedScore = (($totalScore / $totalPossible) * 50) + 50;
                                                @endphp
                                                <div class="text-xs text-gray-500 mt-1">
                                                    {{ number_format($transmutedScore, 2) }}
                                                </div>
                                            @endif
                                        </td>
                                        
                                        <!-- Class Standing (Auto-calculated with weight applied) -->
                                        <td class="px-6 py-4 text-center bg-green-25 border-l border-green-200">
                                            @php
                                                // Calculate weighted class standing dynamically
                                                // For General Education: Always use 66.67% weight
                                                $weightedCS = '-';
                                                if ($totalPossible > 0) {
                                                    // Calculate raw transmuted score
                                                    $rawCS = (($totalScore / $totalPossible) * 50) + 50;
                                                    // Apply General Education weight (66.67%)
                                                    $csWeight = 66.67;
                                                    $weightedCS = number_format($rawCS * ($csWeight / 100), 2);
                                                }
                                            @endphp
                                        ...
                                        <!-- Exam Grade (Auto-calculated dynamically) -->
                                        <td class="px-6 py-4 text-center bg-orange-25 border-l border-orange-200">
                                            @php
                                                // Calculate exam grade dynamically for General Education
                                                $examGrade = '-';
                                                if (isset($termGrade) && $termGrade && $termGrade->exam_score !== null) {
                                                    $examMaxScore = $termGrade->exam_max_score ?? 100;
                                                    // Transmuted formula: (score/max) × 50 + 50
                                                    $rawExam = (($termGrade->exam_score / $examMaxScore) * 50) + 50;
                                                    // Apply weight: 33.33%
                                                    $examGrade = number_format($rawExam * 0.3333, 2);
                                                }
                                            @endphp
                                        ...
                                        <!-- Term Grade (Auto-calculated dynamically) -->
                                        <td class="px-6 py-4 text-center bg-red-25 border-l border-red-200">
                                            @php
                                                // Calculate term grade dynamically
                                                // Term Grade = Weighted CS + Weighted Exam
                                                $termGradeValue = '-';
                                                if ($weightedCS !== '-' && $examGrade !== '-') {
                                                    $termGradeValue = number_format(floatval(str_replace(',', '', $weightedCS)) + floatval($examGrade), 2);
                                                }
                                            @endphp
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTotalCS(input) {
        ...
        if (percentageDisplay && totalPossible > 0) {
            // General Education uses transmuted formula: (score/total) × 50 + 50
            const transmutedScore = ((totalScore / totalPossible) * 50) + 50;
    ...
    function reloadClassStanding(row) {
        // Calculate Class Standing dynamically from Total CS
        const totalCSCell = row.querySelector('td.bg-teal-25');
        const classStandingCell = row.querySelector('td.bg-green-25 .text-sm');
        
        if (!totalCSCell || !classStandingCell) return;
        
        // Get total score and total possible from activity inputs
        const activityInputs = row.querySelectorAll('.grade-input');
        let totalScore = 0;
        let totalPossible = 0;
        ...
        if (totalPossible > 0) {
            // Calculate raw transmuted score: (score/total) × 50 + 50
            const rawCS = ((totalScore / totalPossible) * 50) + 50;
            
            // Apply weight (Always 66.67% for General Education)
            const csWeight = 66.67;
            const weightedCS = rawCS * (csWeight / 100);
            
            // Update Class Standing display
            classStandingCell.textContent = weightedCS.toFixed(2);
        }
        
        // Fetch exam and term grade from server
        ...
        .then(data => {
            // Find and update the Exam Grade cell (has bg-orange-25 class)
            ...
            // Find and update the Term Grade cell (has bg-red-25 class)
