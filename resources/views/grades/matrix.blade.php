    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                <a href="{{ route('grades.term', ['subject' => $subject, 'from' => request()->get('from')]) }}" 
        ...
        <div class="flex items-center space-x-4">
            ...
            <div class="text-left">
                <div class="text-sm text-gray-600 font-medium">
                    ...
                    @if($subject->matrix_type === 'nursing')
                        <span class="ml-2 px-2 py-1 text-xs font-semibold rounded" 
                              :class="subjectType === 'lecture' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'"
                              x-text="subjectType === 'lecture' ? 'LECTURE' : 'LABORATORY'"></span>
                    @endif
                </div>
    ...
    <div x-show="expanded" x-cloak x-transition class="px-6 pb-6 border-t border-gray-200">
        <div class="mt-4 p-4 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg border-2 border-orange-300">
            ...
            <form @submit.prevent="saveFinalRatingWeights()" class="space-y-4">
                @if($subject->matrix_type === 'nursing')
                    <!-- Subject Type Selector for Nursing Matrix -->
                    <div class="bg-white rounded-lg p-4 border-2 border-blue-300">
                        <label class="block text-sm font-medium text-gray-700 mb-3">
                            <i class="fas fa-graduation-cap mr-1 text-blue-600"></i>
                            Subject Type (Nursing Matrix)
                        </label>
                        <div class="flex gap-4">
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" x-model="subjectType" value="lecture" 
                                       class="w-4 h-4 text-blue-600 focus:ring-blue-500">
                                <span class="ml-2 text-sm font-medium text-gray-700">
                                    <i class="fas fa-chalkboard-teacher mr-1 text-blue-600"></i>
                                    Lecture (Final Rating = Final Grade 80% + Comprehensive Exam 20%)
                                </span>
                            </label>
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" x-model="subjectType" value="lab" 
                                ...
                                @if($prelimGrade && $prelimGrade->term_grade !== null)
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            ...
                                            Exam: {{ number_format($prelimGrade->exam_grade ?? 0, 2) }}
                                        </div>
                                    </div>
                                @else
                                ...
                                @if($midtermGrade && $midtermGrade->term_grade !== null)
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            ...
                                            Exam: {{ number_format($midtermGrade->exam_grade ?? 0, 2) }}
                                        </div>
                                    </div>
                                @else
                                    <span class="text-gray-400 text-sm">—</span>
                                ...
                                @if($finalsGrade && $finalsGrade->term_grade !== null)
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            ...
                                            Exam: {{ number_format($finalsGrade->exam_grade ?? 0, 2) }}
                                        </div>
                                    </div>
                                @else
                                    <span class="text-gray-400 text-sm">—</span>
